...
El archivo `generar_grafico.py` contiene una función llamada `generar_grafico`, la cual se encarga de ejecutar las dos soluciones para una lista de tamaños de grilla, medir sus tiempos de ejecución y generar un gráfico comparativo. El gráfico se guarda en un archivo de formato SVG.

Finalmente, el archivo `main.py` es el archivo principal del proyecto. Este archivo llama a la función `generar_grafico`. Al ejecutar este archivo, se generará un gráfico comparando los tiempos de ejecución de ambas soluciones.

El gráfico comparativo generado se guarda en un archivo llamado `tiempos_de_ejecucion_recursivo_vs_PD.svg`. Este archivo contiene el gráfico que muestra el tiempo que tomó cada solución para distintos tamaños de grilla. 
